...
const SimpleQRCode = ({ value, size = 200 }: { value: string; size?: number }) => {
  ...
  const generateQRPattern = (text: string, size: number) => {
    // Simple pattern generation - this is just for demonstration
    const pattern = [];
    ...
    for (let i = 0; i < size; i += 4) {
      for (let j = 0; j < size; j += 4) {
        if ((hash + i + j) % 3 === 0) {
          pattern.push({ x: j, y: i, width: 4, height: 4 });
        }
      }
    }
    return pattern;
      ...
      <View style={styles.content}>
        ...
        <View style={styles.qrContainer}>
          <View style={styles.qrWrapper}>
            <SimpleQRCode value={walletInfo.address} size={250} />
          </View>
